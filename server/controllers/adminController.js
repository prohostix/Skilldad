...
async function createDirector(req, res) {
    try {
        const { name, title, image, order, category, bio, linkedin_url } = req.body;
        const crypto = require('crypto');
        const newId = crypto.randomUUID();
        const result = await query(`
            INSERT INTO directors (id, name, title, image, "order", category, bio, linkedin_url, is_active)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, true) RETURNING *
        `, [newId, name, title, image, order || 0, category || 'DIRECTOR', bio || '', linkedin_url || '']);
...
async function updateDirector(req, res) {
    try {
        const { name, title, image, order, isActive, category, bio, linkedin_url } = req.body;
        const result = await query(`
            UPDATE directors 
            SET name = COALESCE($1, name), title = COALESCE($2, title), image = COALESCE($3, image), 
                ...
                linkedin_url = COALESCE($8, linkedin_url), updated_at = NOW()
            WHERE id = $9 RETURNING *
        `, [name, title, image, order, isActive, category, bio, linkedin_url, req.params.id]);
