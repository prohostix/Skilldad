        ...
        <button
            ...
            className={`w-full flex items-center ${isCollapsed ? 'justify-center p-3' : 'space-x-3 px-4 py-3'} rounded-xl text-sm font-bold font-inter whitespace-nowrap transition-all duration-200 group
                ${isActive
                    ...
                    : 'text-gray-600 dark:text-white/50 bg-transparent hover:bg-[#4C1D95]/5 dark:hover:bg-[#C026FF]/10 hover:text-[#4C1D95] dark:hover:text-[#C026FF]'
                }`}
        >
            <Icon
                size={20}
                className={`flex-shrink-0 transition-colors ${isActive ? 'text-[#4C1D95] dark:text-[#C026FF]' : 'text-gray-500 dark:text-white/50 group-hover:text-[#4C1D95] dark:group-hover:text-[#C026FF]'}`}
            />
...
const HomeNavItem = ({ onClick, isCollapsed }) => {
    return (
        <button
            ...
            className={`w-full flex items-center ${isCollapsed ? 'justify-center p-3' : 'space-x-3 px-4 py-3'} rounded-xl border border-gray-200 dark:border-[#C026FF]/20 text-gray-700 dark:text-white/60 hover:text-[#4C1D95] dark:hover:text-white hover:bg-gray-50 dark:hover:bg-[#C026FF]/10 hover:border-[#4C1D95]/30 text-sm font-bold font-inter transition-all duration-200 group`}
        >
            <Home size={20} className="flex-shrink-0 text-gray-500 dark:text-white/60 group-hover:text-[#4C1D95] dark:group-hover:text-[#C026FF] transition-colors" />
                                    ...
                                    <div key={item.name} className="space-y-1">
                                        ...
                                        >
                                            <div className="flex items-center space-x-3">
                                                <Icon size={20} className={`flex-shrink-0 transition-colors ${isAnySubActive ? 'text-[#4C1D95] dark:text-[#C026FF]' : 'text-gray-500 dark:text-white/50 group-hover:text-[#4C1D95] dark:group-hover:text-[#C026FF]'}`} />
                                                {!isCollapsed && <span>{item.name}</span>}
                                            </div>
                                            {!isCollapsed && (
                                                universitiesDropdownOpen ? <ChevronDown size={16} className="text-gray-400" /> : <ChevronRight size={16} className="text-gray-400" />
                                            )}
                                        </button>
                                                            ...
                                                            <button
                                                                ...
                                                                className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium font-inter transition-all duration-200
                                                                    ${isSubActive
                                                                        ...
                                                                        : 'text-gray-500 dark:text-white/40 hover:text-[#4C1D95] dark:hover:text-[#C026FF] hover:bg-[#4C1D95]/5 dark:hover:bg-[#C026FF]/5'
                                                                    }`}
                                                            >
                                                                {sub.name}
                                                            </button>
