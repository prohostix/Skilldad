...
/**
 * MeetingRecordingPlayer Component
 * Plays session recordings in an embedded player.
 * If recordingUrl is not provided, it can fetch it using sessionId.
 * 
 * @param {string} recordingUrl - Recording play URL (optional)
...
const MeetingRecordingPlayer = ({ recordingUrl, sessionId, title, onEnded, onError }) => {
  ...
  const fetchPlaybackUrl = async () => {
    try {
      ...
      console.error('[Recording Player] Error fetching playback URL:', err);
  ...
  const handleVideoError = (e) => {
    console.error('[Recording Player] Error loading video:', e);
  ...
  const handleVideoEnded = () => {
    console.log('[Recording Player] Video ended');
    if (onEnded) {
      onEnded();
    }
  };
  ...
  const isExternalEmbed = url && (url.includes('zoom.us/rec') || url.includes('vimeo.com') || url.includes('youtube.com'));
    ...
    <div className="relative w-full aspect-video bg-black rounded-lg overflow-hidden">
      ...
      <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-white/10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-[10px] font-bold uppercase tracking-wider">Session Recording</span>
        </div>
      </div>
    </div>
  );
